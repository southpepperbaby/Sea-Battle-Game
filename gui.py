...
class BattleshipGUI:
    ...
    def create_field_canvas(self, parent, title, clickable=False, click_callback=None):
        ...
        if clickable and click_callback:
            canvas.bind(
                "<Button-1>",
                lambda e: self.on_field_click(e, click_callback),
            )
    ...
    def update_ship_info(self):
        ...
        if self.current_ship_index < len(self.ships_to_place):
            ...
            self.ship_info_label.config(text="Все корабли расставлены!")
    ...
    def place_ship_click(self, x, y):
        ...
        if self.current_board.place_ship(x, y, length, self.horizontal):
            ...
            self.draw_field(
                self.placement_canvas,
                self.current_board.get_field_for_display(),
            )
            self.update_ship_info()
            if self.current_ship_index >= len(self.ships_to_place):
    ...
    def shoot_click(self, x, y):
        """Обработка выстрела (режим 2 игрока)"""
    ...
    def start_bot_game(self):
        """Начать игру после расстановки игрока (против бота)"""
        self.game.player_setup_complete()
        self.setup_game_screen_vs_bot()

    def setup_game_screen_vs_bot(self):
        ...
        frame1, self.enemy_canvas = self.create_field_canvas(
            ...
            "Поле компьютера",
            clickable=True,
            click_callback=self.shoot_click_vs_bot,
        )
        frame1.pack(side=tk.LEFT, padx=20)
    ...
    def shoot_click_vs_bot(self, x, y):
        """Обработка выстрела игрока по боту"""
        ...
        messages = {
            "miss": "Ваш выстрел: мимо!",
            "hit": "Ваш выстрел: попадание!",
            "destroy": "Ваш выстрел: корабль уничтожен!",
            "win": "ВЫ ПОБЕДИЛИ!",
            "invalid": "Сюда уже стреляли!",
        }
        ...
        if result == "win":
            self.show_winner("Игрок")
            return

        self.update_game_display()

        if not self.game.game_over and self.game.current_player == 2:
    ...
    def bot_move(self):
        ...
        messages = {
            "miss": "Ход бота: мимо!",
            "hit": "Ход бота: попадание!",
            "destroy": "Ход бота: ваш корабль уничтожен!",
            "win": "БОТ ПОБЕДИЛ!",
        }
